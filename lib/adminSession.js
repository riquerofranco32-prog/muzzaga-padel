...
/**
 * Sin ADMIN_PASSWORD en producción no hay login posible: una contraseña por
 * defecto conocida permitiría entrar y además firmar cookies falsas.
 */
function adminSecret() {
  const secret = (process.env.ADMIN_PASSWORD || "").trim();
  if (secret) return secret;
  return process.env.NODE_ENV === "production" ? null : "muzzaga2026";
}
...
function signingKey() {
  ...
  return createHmac("sha256", secret)
    .update("muzzaga:admin:session:v1")
    .digest();
}
...
export async function requireAdmin() {
  ...
  return {
    ok: false,
    error: "Sesión expirada o inválida. Volvé a ingresar al panel.",
  };
}
