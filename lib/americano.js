...
export function buildAmericanoShareMessage({ names = [], fixtures = [], scores = {} }) {
  let msg = `*🏆 TORNEO AMERICANO EXPRESS — MUZZAGA PÁDEL*\n`;
  msg += `Participantes (${names.length}): ${names.join(", ")}\n\n`;

  fixtures.forEach((f) => {
    const score = scores && scores[f.round];
    const scoreText = score && (score.t1 || score.t2) ? ` [${score.t1 || 0} - ${score.t2 || 0}]` : "";
    msg += `• *Ronda ${f.round}:* ${f.p1} & ${f.p2} vs ${f.p3} & ${f.p4}${scoreText}\n`;
    if (f.bye) msg += `   _Descansa: ${f.bye}_\n`;
  });

  msg += `\n🍺 Tercer tiempo y premiación en la cantina de Muzzaga`;
  return msg;
}

export function buildAmericanoWhatsAppUrl(params) {
  return `https://wa.me/?text=${encodeURIComponent(buildAmericanoShareMessage(params))}`;
}
