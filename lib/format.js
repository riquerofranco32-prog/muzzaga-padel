...
/** Para buscar sin que importen tildes ni mayúsculas: "Martín" ~ "martin". */
...
/** "hoy", "ayer", "hace 5 días", "hace 2 meses" entre dos fechas ISO. */
export function formatRelativeDays(isoDate, todayIso) {
  if (!isoDate) return "—";
  const ms = Date.parse(`${todayIso}T00:00:00Z`) - Date.parse(`${isoDate}T00:00:00Z`);
  const days = Math.round(ms / 86_400_000);
  if (days < 0) return days === -1 ? "mañana" : `en ${-days} días`;
