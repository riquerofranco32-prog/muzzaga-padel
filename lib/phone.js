...
/**
 * "+54 9 299 555-1234" para mostrar. Asume código de área de 3 dígitos
 * salvo AMBA (11).
 * ponytail: las áreas de 4 dígitos (ej. 2942) se muestran partidas distinto;
 * si hace falta exactitud, sumar la tabla de códigos de área.
 */
export function formatPhoneAR(raw) {
  const wa = toWhatsappNumber(raw);
  if (wa.length !== 13) return (raw || "").trim();
  const national = wa.slice(3); // 10 dígitos
  ...
  return `+54 9 ${area} ${rest.slice(0, cut)}-${rest.slice(cut)}`;
}
